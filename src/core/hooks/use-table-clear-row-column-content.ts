...
import { CellSelection, cellAround, deleteCellSelection } from "prosemirror-tables";
...
import {
  ...
  type Orientation,
  setCellAttr,
} from "@/extensions/table-handle/utils";
// --- Hooks ---
...
function resetCellAttributes(editor: Editor): boolean {
  try {
    return setCellAttr(DEFAULT_CELL_ATTRS)(editor.state as any, editor.view.dispatch as any);
  } catch (error) {
...
function clearSelectedCells(editor: Editor, resetAttrs: boolean = false): boolean {
  try {
    ...
    if (selection instanceof CellSelection) {
      ...
      deleteCellSelection(editor.state as any, editor.view.dispatch as any);
